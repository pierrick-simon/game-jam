...
@export var air_jump_velocity: float = -350.0
@export var water_jump_velocity: float = -700.0
...
func handle_jump(body: CharacterBody2D, want_to_jump: bool, is_dashing: bool, is_underwater: bool) -> void:
	if has_just_landed(body):
		is_jumping = false
	if is_dashing:
		body.velocity.y = 0
	if want_to_jump and (body.is_on_floor() or not coyote_timer.is_stopped()) and not is_dashing:
		jump(body, is_underwater)
	
	handle_coyote_time(body)
	handle_jump_buffer(body, want_to_jump, is_dashing, was_dashing, is_underwater)
	
	is_going_up = body.velocity.y < 0 and not body.is_on_floor()
	was_dashing = is_dashing
	last_frame_on_floor = body.is_on_floor()

func handle_jump_buffer(body: CharacterBody2D, want_to_jump: bool, is_dashing: bool, was_dashing: bool, is_underwater: bool) -> void:
...
func jump(body: CharacterBody2D, is_underwater: bool) -> void:
	body.velocity.y = (water_jump_velocity) if is_underwater else air_jump_velocity
